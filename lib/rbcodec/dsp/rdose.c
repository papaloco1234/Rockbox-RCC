...
void dsp_rdose_enable(int var)
{
    ...
    //if(rDose_enabled && !dsp_proc_active(dsp,DSP_PROC_RDOSE))
    //    dsp_proc_activate(dsp, DSP_PROC_RDOSE, true);
    //if(!rDose_enabled && dsp_proc_active(dsp,DSP_PROC_RDOSE))
    //    dsp_proc_activate(dsp, DSP_PROC_RDOSE, false);
}
     ...
     if (no_repair == false)
     {
        for (i = 0; i < count; i++)
        {
            ...
            if (sample_gain != -1)
            {
                ...
                x = FRACMUL_SHL(sample_gain, x, 7);
                temp_buffer[0][i] += x;
                y = FRACMUL(temp_buffer[1][i], tcoef5);
                y = FRACMUL_SHL(sample_gain, y, 7);
                temp_buffer[1][i] += y;
            }
         } 
     }
