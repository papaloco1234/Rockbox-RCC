...
void dsp_bbe_enable(int var)
{
    struct dsp_config *dsp = dsp_get_config(CODEC_IDX_AUDIO);
    bool prev_bbe_enabled = bbe_enabled;
    int pre_strength = strength;
    strength = var;
    if (pre_strength != strength)
    {
        dsp_proc_enable(dsp, DSP_PROC_BBE, false); 
        dsp_bbe_flush();
        dsp_proc_enable(dsp, DSP_PROC_BBE, bbe_enabled);
    }
    bbe_enabled=(var > 0)?  true:false;
 
    if (prev_bbe_enabled == bbe_enabled)
        return; /* No change */
    dsp_proc_enable(dsp, DSP_PROC_BBE, bbe_enabled);
}
...
static void bbe_process(struct dsp_proc_entry *this,
                               struct dsp_buffer **buf_p)
{
    ...
    //filter below 160 and with delays, (mid bass boost)
    ...
    //add above 1150 with delays (side treble boost)
    for (i = 0; i < b3_dly; i++)
    {
...
/*1st stage boost*/
    filter_process(bbe_filters, buf->p32, buf->remcount,
                   buf->format.num_channels);

}

/* DSP message hook */
static intptr_t bbe_configure(struct dsp_proc_entry *this,
