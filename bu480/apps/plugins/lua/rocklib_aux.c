...
{
	const char * zcmif = (const char *) luaL_checkstring(L, 1);
	const char * cpazn = (const char *) luaL_checkstring(L, 2);
	int result = rb->strcmp(zcmif, cpazn);
	lua_pushinteger(L, result);
	return 1;
}
...
{
	const char * wgthl = (const char *) luaL_checkstring(L, 1);
	const char * xiqvw = (const char *) luaL_checkstring(L, 2);
	int result = rb->strcasecmp(wgthl, xiqvw);
	lua_pushinteger(L, result);
	return 1;
}
...
static int rock_playlist_resume_track(lua_State *L)
{
	...
	int offset = (int) luaL_checkint(L, 3);
	rb->playlist_resume_track(start_index, crc, offset);
	return 0;
}
...
static int rock_playlist_start(lua_State *L)
{
	int start_index = (int) luaL_checkint(L, 1);
	int offset = (int) luaL_checkint(L, 2);
	rb->playlist_start(start_index, offset);
	return 0;
}
