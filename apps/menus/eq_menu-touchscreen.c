                           ...
                           struct eq_screen_data  *eq_data)
{
    ...
    btn_bar->cancel.width = btn_bar->edit_mode.x - btn_bar->cancel.x - 5;

    /*Minus button*/
    snprintf(buf, sizeof(buf), " - ");
    screen->getstringsize(buf, &w, &h);
    btn_bar->minus.x = 0;
    btn_bar->minus.width = vp->width / 2 - 5;
