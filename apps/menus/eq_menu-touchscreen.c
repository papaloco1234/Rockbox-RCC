...
struct button_bar{
    ...
    struct button_data plus;
    struct button_data minus;
    short height;
    short y;
};
...
#define COLOUR1 LCD_RGBPACK(255, 85, 0)
#define COLOUR2 LCD_RGBPACK(51, 51, 51)
#define TXT_COLOUR LCD_RGBPACK(108, 122, 137)
#define COLOUR_WHITE LCD_RGBPACK(255, 255, 255)
#define COLOUR_DARK LCD_RGBPACK(45, 45, 45)
#define BAR_FG  COLOUR1
#define BAR_BG  LCD_RGBPACK(204, 204, 204)
...
#define RIGHT_EDGE 2
    ...
    screen->set_foreground(BAR_FG);
    screen->fillrect(inner_x, inner_y, start - inner_x, inner_ht);

    screen->set_foreground(BAR_BG);
    screen->fillrect(start+size, inner_y, inner_wd - start + inner_x - size, inner_ht);

    /* draw slider */
    screen->set_foreground(COLOUR_DARK);
    screen->fillrect(start, inner_y, size, inner_ht);
    screen->set_foreground(COLOUR_WHITE);
    screen->fillrect(start + 1, inner_y + 1, size - 2, inner_ht - 2);
                            ...
                            enum eq_slider_mode mode)
{
    ...
    for (band = 0; band < EQ_NUM_BANDS; band ++) {
        ...
        screen->set_foreground(TXT_COLOUR);
        ...
        if (mode != CUTOFF && mode != Q)
            draw_slider_bar(screen, eq_data->band.x, eq_data->band.y[band] + eq_data->slider.y,
                       eq_data->band.width, eq_data->slider.height, value,
                       min_item, max_item);
        else /* for Q/CUTOFF draw a blank bar. Better use minus/plus buttons than slider */
        {
            screen->set_foreground(BAR_BG);
	        screen->fillrect(eq_data->band.x, eq_data->band.y[band] + eq_data->slider.y,
...
static void draw_button_filled (struct screen *screen, char *buf,
                         int x, int y, int width, int height, int offset, unsigned colour)
{
    int txt_w, txt_h;
    unsigned fg = screen->get_foreground();
    unsigned bg = screen->get_background();

    screen->set_foreground(colour);
    /* draw button */
    screen->fillrect(x, y, width, height);

    screen->set_foreground(COLOUR_WHITE);
    screen->set_background(colour);
    /* draw button text */
    screen->getstringsize(buf, &txt_w, &txt_h);
    screen->putsxy(x + (width - txt_w)/2 , (height - txt_h)/2 + offset, buf);
    screen->set_foreground(fg);
                            ...
                            struct button_bar *btn_bar)
{
    ...
    draw_button_filled(screen, buf, btn_bar->edit_mode.x, 0,
                        btn_bar->edit_mode.width - 5, btn_bar->height, 0, 
                        COLOUR2);

    /* Cancel button */
    snprintf(buf, sizeof(buf), str(LANG_KBD_CANCEL), NULL, NULL);
    draw_button_filled(screen, buf, btn_bar->cancel.x, 0,
                        btn_bar->cancel.width - 10, btn_bar->height, 0, COLOUR2);

    /* Ok button */
    snprintf(buf, sizeof(buf), str(LANG_KBD_OK), NULL, NULL);
    draw_button_filled(screen, buf, btn_bar->ok.x, 0,
                        btn_bar->ok.width - 5, btn_bar->height, 0, COLOUR1);
    /*Plus button*/
    snprintf(buf, sizeof(buf), " + ");
    draw_button_filled (screen, buf, btn_bar->plus.x, btn_bar->height * 9,
                        btn_bar->plus.width,
                        btn_bar->height, btn_bar->height * 9,
                        (long_press == 2)? COLOUR1:COLOUR2);

    /*Minus button*/
    snprintf(buf, sizeof(buf), " - ");
    draw_button_filled (screen, buf, btn_bar->minus.x, btn_bar->height * 9,
                        btn_bar->minus.width,
                        btn_bar->height, btn_bar->height * 9,
                        (long_press == 1)? COLOUR1:COLOUR2);
    return;
}
                           ...
                           struct eq_screen_data  *eq_data)
{
    ...
    btn_bar->edit_mode.width = w + w * 0.05;
    ...
    screen->getstringsize(buf, &w, &h);
    btn_bar->ok.x = 0 + 5;
    btn_bar->ok.width = w  * 2;

    /* Cancel button */
    snprintf(buf, sizeof(buf), str(LANG_KBD_CANCEL), NULL, NULL);
    screen->getstringsize(buf, &w, &h);
    btn_bar->cancel.width = btn_bar->edit_mode.x  - btn_bar->ok.width;
    btn_bar->cancel.x = btn_bar->ok.width + 5;

    /*Minus button*/
    snprintf(buf, sizeof(buf), " - ");
    screen->getstringsize(buf, &w, &h);
    btn_bar->minus.x = 0 + 5;
    btn_bar->minus.width = vp->width / 2 - 10;

    /*Plus button*/
    snprintf(buf, sizeof(buf), " + ");
    screen->getstringsize(buf, &w, &h);
    btn_bar->plus.width = vp->width / 2 - 5;
    btn_bar->plus.x = vp->width / 2;


    screen->getstringsize("A", &w, &h);
    if (w * 20 > vp->width ||
        (btn_bar->cancel.width + btn_bar->ok.width +
         btn_bar->edit_mode.width) > vp->width)
    {
        return false; /* buttonbar or 3 settings do not fit on the screen */
    }

    /* determine the height of the button bar */
    btn_bar->height = vp->height * 0.1;

    /* calculate band postion and size */
    eq_data->band.x = vp->width * 0.05 + BUTTON_MARGIN;
    eq_data->band.width = vp->width * 0.9 - BUTTON_MARGIN * 2;
    eq_data->band.height = (vp->height - 1 - btn_bar->height * 2 -
    ...
    if (eq_data->slider.height > vp->height * 0.016)
    {
        eq_data->slider.height = vp->height * 0.016;
    }
    else if (eq_data->slider.height < vp->height * 0.016)
    {
...
bool eq_menu_graphical(void)
{
    ...
    unsigned bg = screens[SCREEN_MAIN].get_background();
    unsigned fg = screens[SCREEN_MAIN].get_foreground();
    ...
    while (!exit_request) {
        ...
        screens[SCREEN_MAIN].set_foreground(COLOUR_WHITE);
        screens[SCREEN_MAIN].fillrect(0, 0,
                       vp[SCREEN_MAIN].width, vp[SCREEN_MAIN].height); 
        screens[SCREEN_MAIN].set_background(COLOUR_WHITE);
        ...
        draw_button_bar(&screens[SCREEN_MAIN], &vp[SCREEN_MAIN], mode,
                                                long_press, &btn_bar);
...
#define EQ_SLIDER_TCONST 2
            ...
            slider_change)
        {
            ...
            {   /* convert touch into VP coordinates */
                ...
                else if (touch_x >= btn_bar.minus.x &&
                         touch_y >= btn_bar.height * 9 &&
                         touch_x <= btn_bar.minus.x + btn_bar.minus.width)
                {
                ...
                else if (touch_x >= btn_bar.plus.x &&
                         touch_y >= btn_bar.height * 9 &&
                         touch_x <= btn_bar.plus.x + btn_bar.plus.width)
                {
                    if  (button_touchscreen & BUTTON_REL)
                        button = BUTTON_NONE;
                    else
    ...
    screens[SCREEN_MAIN].set_background(bg);
    screens[SCREEN_MAIN].set_foreground(fg);
